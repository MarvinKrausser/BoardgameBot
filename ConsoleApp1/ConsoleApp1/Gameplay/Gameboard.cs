...
public class Gameboard
{
    ...
    private readonly List<LembasFieldTile> lembasFieldTiles;
    ...
    public BoardState BoardState
    {
        ...
        {
            var i = 0;
            var tempLembasField = new List<LembasField>();
            foreach (var lembasFieldTile in lembasFieldTiles)
            {
                var temp = new LembasField(new[] { lembasFieldTile.X, lembasFieldTile.Y },
    ...
    private Tile[,] createBoard()
    {
        ...
        foreach (var wall in boardConfig.walls) tiles[wall[0][0], wall[0][1]].setWall(tiles[wall[1][0], wall[1][1]]);
    ...
    public List<Tile> GetNeighbors(Tile tile)
    {
        ...
        if (tile.X != 0 && !tile.hasWall(directionEnum.WEST)) neighbors.Add(tiles[tile.X - 1, tile.Y]);

        if (tile.X != Width - 1 && !tile.hasWall(directionEnum.EAST)) neighbors.Add(tiles[tile.X + 1, tile.Y]);

        if (tile.Y != 0 && !tile.hasWall(directionEnum.NORTH)) neighbors.Add(tiles[tile.X, tile.Y - 1]);

        if (tile.Y != Width - 1 && !tile.hasWall(directionEnum.SOUTH)) neighbors.Add(tiles[tile.X, tile.Y + 1]);
