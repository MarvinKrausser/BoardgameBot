...
public class Cards
{
    ...
    private readonly List<cardEnum> _nachziehstapel; // initial: 20 cards
    ...
    private void setInitialNachziehstapelCards()
    {
        ...
        for (var i = 0; i < 3; i++) _nachziehstapel.Add(cardEnum.MOVE_2); //adding 3 "Move 2" cards

        for (var i = 0; i < 5; i++) _nachziehstapel.Add(cardEnum.MOVE_1); //adding 5 "Move 1" cards

        _nachziehstapel.Add(cardEnum.MOVE_BACK); //adding 1 "Move Back" card
        _nachziehstapel.Add(cardEnum.U_TURN); //adding 1 "U-Turn" card
        for (var i = 0; i < 3; i++) _nachziehstapel.Add(cardEnum.RIGHT_TURN); //adding 3 "Right Turn" cards

        for (var i = 0; i < 3; i++) _nachziehstapel.Add(cardEnum.LEFT_TURN); //adding 3 "Left Turn" cards

        for (var i = 0; i < 2; i++) _nachziehstapel.Add(cardEnum.AGAIN); //adding 2 "Again" cards
    ...
    private void shuffleDeck(List<cardEnum> stack)
    {
        ...
        {
            n--;
            var k = rng.Next(n + 1); //Next(MaxValue): returns a not negativ random integer, that is lower than MaxValue
            var switchVariable = stack[k];
            stack[k] = stack[n];
    ...
    {
        var playerLifes = lifes; //max 3 min 0 
        ...
        if (playerLifes > 0 && playerLifes < 4)
        {
            ...
            for (var i = 0; i < numberOfCards; i++) //selects 7 or 8 or 9 random cards as "_handkarten"
            {
    ...
    {
        for (var i = 0; i < _ablagestapel.Count; i++) _nachziehstapel.Add(_ablagestapel[i]);
    ...
    ///     3) checks if the player logged none or not all five cards in. if there are less than 5 cards, this method adds the
    ///     rest
    /// </summary>
    ...
    public bool playersCardChoice(List<cardEnum> cards)
    {
        ...
        var loggedInCards = new List<cardEnum>();
        var notLoggedInCards = new List<cardEnum>();

        for (var i = 0; i < _handkarten.Count; i++) //initial: all "_handkarten" are not logged in
            notLoggedInCards.Add(_handkarten[i]);

        for (var i = 0; i < 5; i++) //It should be: cards.Count == 5. If not, then the rest will be ignored
        {
            ...
            {
                if (!cards[i].Equals(cardEnum.EMPTY)) returnValue = false;
        ...
        while (counter < 5)
        {
            if (_nachziehstapel.Count <= 0 && loggedInCards[counter].Equals(cardEnum.EMPTY))
            {
                ...
                for (var i = 0; i < 5; i++) //add 5 EMPTY cards, which will be replaced later 
    ...
    {
        for (var i = 0; i < cards.Count; i++) _ablagestapel.Add(cards[i]);
    }
